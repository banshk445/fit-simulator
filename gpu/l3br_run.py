"""v4-15 §1-①② — **넓이 검증 실행기**(칸 하나). `l3full_run.py`(v4-14)와 «같은 계기»이되
**칸을 인자로 받는다**. 물리 식 0줄 · `engine/` 9파일 바이트 불변.
...
진입: `py gpu/l3br_run.py <cuda|x64> <f64|f32> <cell>`
"""
import json
import struct
import sys
...
out = load.EXPORT / f"l3br-{CELL}-{FPN}-{ARCH}"
