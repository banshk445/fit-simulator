 ...
 *   문턱    **일반형 = `h × 25%`**(도출식). 출처는 `instruments.ts` 정정 **정본 문언**
 *           「격자 SDF 오차는 **최대 h의 약 25%**」이고, **스펙 «문장»을 게이트로 승격**한 것이다(**새 수 0**).
 *           **v3-70 §3 한정 재등재 — 구 문언 무삭제**:
 *             구 「v3-56 «고정 승격» — `h × 25%`(현행 h 3.951mm ⟹ **0.9877mm**)」
 *             ⟹ **0.9877mm 는 «기본 몸 «전용»» 예시값**이다. `h` 는 `deriveSpacing(BEXT, …)` 의
 *                출력이고 **`BEXT` 는 몸에서 온다**(v3-69 §1-3 ② · §1-5) ⟹ **몸이 바뀌면 문턱도 바뀐다**.
 *             ★ **실측 1건(v3-70)**: **코드는 처음부터 도출식이었다** —
 *                `scripts/v3Gate3b.ts:51` `const TH = TH_FRAC * P.sdfSpec.h * 1000`.
 *                고정돼 있던 것은 **«문언의 예시 수»뿐**이고 **하드코딩된 문턱은 없었다**.
 *                ⟹ **「도출 복원」에 «코드 변경 0줄»** — 이 판이 고친 것은 **문언**이다(**완화 0**).
 *           **구 문턱**(v3-53 §2-1 «밴드 안 최대차» 원단별 인용)은 **무삭제 · 대조 전용**으로 남는다.
 *   판정    **max Δ ≤ 문턱 ⟹ 통과.**
 *   ★판별력 **구 문턱은 «판정 정의역과 같은 표본»에서 나와 구조적 항등**이었다(v3-55 §2-1 · gray 동률).
 *           **신 문턱은 h(굽기 스펙)에서 오므로 정의역과 «서로소»** ⟹ **실제로 떨어질 수 있다**.
 *           ⟹ **[[함정 #117]] 「문턱 출처와 정의역은 서로소여야 판별력이 생긴다」의 해소분이다.
 * ══════════════════════════════════════════════════════════════════════════════ */
