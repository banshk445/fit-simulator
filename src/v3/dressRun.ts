...
export type RunInput = {
  ...
  asm2?: boolean;
  /** ★ v5-22 — 1세대 배치 + 앵커 정정 + `SH_DROP`(그대로 넘긴다 · 기본 off). */
  asm1x?: boolean | 'anchor' | 'drop';
  /** ★ C1 — 옷깃 후보 계열(그대로 넘긴다 · 없으면 현행). */
  c1Var?: string;   // ★ v5-25 — 그대로 넘긴다
...
export function prepare(inp: RunInput) {
  ...
  const S = createScene({
    ...
    ...(inp.asm1x ? { asm1x: inp.asm1x } : {}),
    ...(inp.c1Var ? { c1Var: inp.c1Var } : {}),      // ★ C1             // v5-22 — 같은 규약
  });
