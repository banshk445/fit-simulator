...
- **소매캡 틈**: 어깨 부근에서 맨살이 비친다. **진단 완료(2026-07-29)** —
  천에 구멍이 있는 게 아니다(용접 seamGap 72값 전부 0.00cm). 천이 몸
  **안으로** 최대 2.7cm 파묻혀 마네킹이 천 앞에 그려지는 것이다.
...
- **뒤 목선 국소 구김**: **진단 완료(2026-07-29)** — 한 가지가 아니라 두
  층이 같은 자리에 겹쳐 있다. (1) 목 구멍 가장자리 열의 row0 꺾임
  (곡률 10.1mm)은 **순수 패턴 기하**다 — row0은 하드 핀이라 물리 기여분이
  0이고, `necklineRise`의 C¹ 단절 + `necklineLift` 봉우리로 전부 설명된다.
  (2) 뒤판 열 12~13/30~31, 행 1~3의 곡률 띠는 물리 층이고, 뿌리가 목선이
  아니라 **뒤판 접촉 부재**(tb hit 0.634)다 — M2 동결 상태에 묶여 있어
