...
> 배포된 사이트에서 찍었다(긴팔 기본값 · `DONE f=322`). **이 화면의 9채널은 같은 실행에서
> 정본과 17/17 대조를 마쳤다** — 그림과 숫자가 같은 실행에서 나왔다.
> 핏 맵을 끈 원단 그대로의 화면은 `docs/captures/P29-매듭갱신/배포-체커-핏맵off.jpg`.
...
| **백그라운드 탭에서는 시작할 수 없다** | 착장 루프 자체는 워커의 동기 루프라 탭 가시성과 무관하다. 그런데 **시작 조건**은 마네킹 포즈가 멎었는지를 보고, 그 포즈 루프는 메인 스레드 rAF다 — 백그라운드 탭은 rAF가 멎어 프레임이 0이고 「착장하기」가 안 켜진다. 대기 상한 탈출은 그 조건 **뒤에** 있어 발화하지 못한다(`DressButton.tsx`). 탭을 전경으로 되돌리면 정상 진행되므로 **갇히지는 않는다** |
