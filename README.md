...
상태다 — 재촬영 보류.** 이유: (a) 목선 변경분은 그 프레이밍에서 약 2mm라
눈에 안 잡힌다(교체하면 보이지도 않는 개선을 주장하는 셈) (b) 소매 콜라이더
배선은 화면 회귀가 나와 채택 보류 상태라, 지금 찍으면 되돌릴 수도 있는
상태가 박제된다. **소매 판단이 끝난 뒤 9장을 한 번에 다시 찍는다.**
...
측정 쪽 6건, 제어 쪽 1건, 측정 대상 쪽 1건, 시간 쪽 1건, 기록 쪽 1건. 전부 실제로 당한 것이고,
...
착장, PBD 드레이프(용접 단일 메시 + SDF 마찰 + 렌더 세분화), 핏 맵
...
- **소매캡 틈**: 어깨 부근에서 맨살이 비친다. **진단 완료(2026-07-29),
  ...
  **1차 시도(소매에 몸 메시 리졸버 배선)는 지표만 개선하고 화면은
  악화**시켰다 — 관통 정점 18.1→15.3%인데 오른쪽 어깨 노출은 되레 확대
  (`docs/captures/sleeve-collider/`). 계기가 부호만 세고 노출 면적을 안
  재서 못 걸렀다. 진단·측정 전문은 `docs/metrics-log.md`
  (2026-07-29 △3-3 블록 / 2026-07-30 화면 판정 블록).
