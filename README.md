...
>
> 기준선 수치와 대조하려면 **`?patterncore=1&ringtotal=0`** 으로 들어가야 한다.
> 목선 링 총 길이 상한을 교란 요인에서 뺀 상태가 계기의 정의역이고, 이 파라미터 없이
> 잰 값은 기준선과 **비교할 수 없다**(같은 실행에서 링 60.70 → 54.05cm로 갈린다).
...
- **몸 5축 · 옷 5축이 전부 살아 있다** — 키·가슴·팔·다리·어깨너비(몸) /
  ...
  (그림을 늘이는 것이 아니다).
- **핏 맵** — 「핏 맵」을 켜면 옷 표면이 **눌림(빨강) / 밀착(노랑) / 여유(파랑)**로 칠해진다.
  경계는 표와 **같은 값**(흡착 margin 15.0mm)이고, 부호거리를 못 구한 자리는 회색으로 둔다 —
  없는 값을 색으로 지어내지 않는다.
...
- **같은 치수면 같은 결과가 나온다** — 마네킹 포즈를 누적 회전에서 절대 구성으로
  바꿔(P27) 27회 실행이 전부 **비트 동일**하고 착장 실패가 0/27이다. 슬라이더를
  움직였다 되돌린 뒤에도 처음 값으로 돌아온다. 그 전까지는 같은 입력이 실행마다
  갈렸다 — 이 저장소에서 가장 오래 걸린 수리다(P21~P27).

![핏 맵 + 핏 리포트 — 부위 5행(소매 포함) · 긴팔 DONE f=322](docs/captures/P29-매듭갱신/배포-핏맵+리포트.jpg)

![같은 실행 · 핏 맵을 끈 원단 그대로 — 드레이프](docs/captures/P29-매듭갱신/배포-체커-핏맵off.jpg)

> 배포된 사이트에서 찍었다(긴팔 기본값 · `DONE f=322`). **두 장은 같은 실행이고,
> 그 실행의 9채널은 그 자리에서 정본과 17/17 대조를 마쳤다** — 그림과 숫자가 같은
> 실행에서 나왔다. 위는 핏 맵을 켠 화면(색 + 리포트), 아래는 끈 화면(원단 그대로 ·
> 드레이프가 읽힌다).
...
| **배치** | `src/lib/patternGarment.ts` | 패턴을 삼각화하고 몸 앞뒤 평면·팔 축 원통으로 **3D에 놓는다**(시접 쌍 포함). 패널 수·구성은 고정이 아니다 — 소비 층이 **역할을 묻는다**(`topology.roleOf/panelsWithRole`, P18) |
...
RINGTOTAL=0 PATTERNCORE=1 npm run dress:pattern   # 같은 물리를 Node에서(계측 로그 934줄)

# v1 게이트 4종
npm run check:weld && npm run check:sleeve && npm run check:seambridge && npm run check:demo
# v2 게이트 4종 (제도·삼각화·배치 / 경계 / 외곽선 / 패널 위상)
PATTERNCORE=1 npm run check:pattern
PATTERNCORE=1 npm run check:boundary && PATTERNCORE=1 npm run check:outline
PATTERNCORE=1 npm run check:topology   # 티셔츠 4패널 · 셔츠형 5패널 · 커프형 6패널

npx tsc -b && npm run build
```

`check:pattern`은 **실패 1건이 선 채로 유지된다**(소매 최소각 24.74° < 문턱 25°).
삼각화 정련 상수는 엣지 균일도·최소각·정점 예산이 서로를 잡아먹는 구조라 대역 안에서
셋을 동시에 만족시키지 못했다. **문턱을 결과에 맞춰 내리지 않는다** — 실패인 채로 두고
그 사실을 게이트가 매번 말하게 한다.
...
- **md5 + 로그 934줄 diff 게이트.** 리팩터가 항등인지 확인할 때 최종 좌표 해시만으로는
  부족하다 — 계측 인쇄 934줄을 통째로 비교한다(시간만 마스킹). 실제로 md5는 통과하고
  로그가 잡은 결함이 있었다.
- **단계별 비트 해시.** 값이 실행마다 갈릴 때 "어디서부터" 갈리는지 묻는 채널이 따로
  있다 — 몸 메시·몸 인덱스·팔 관절·팔 축·제도·삼각형·엣지·배치·정착을 각각 해시한다
  (`__fitDebug.dressMetrics().diag.bits`). 이 채널이 비결정성의 자리를 **제도**로
  좁혔고, 증폭기가 `round(total)+1`의 정수 경계라는 것까지 값으로 확정했다(P21).
...
- **기준선 두 벌.** A = 고정 fixture(회귀용) · B = 살아있는 마네킹(제품용).
  ...
  현재 정본은 **B-3**(반팔 · 새 로드 기본값): cov 6.94%(120/1728) · maxStrain 2.6505 ·
  maxSeamGap 10.515mm · Δ20 5.429mm · 자기교차 1777 · 관통 51/5204 · 밑단 합 114.146cm ·
  목선 링 60.698cm · 소맷부리 35.284cm · 정점·제약 5204·29392 · DONE f=369.
  긴팔은 cov 6.54%(114/1744) · 자기교차 1969 · 관통 107/6294 · 링 61.414cm ·
  소맷부리 25.810cm · 정점·제약 6294·35738 · DONE f=322.
  전문은 `docs/product-axis/P28-기준선재수립.md`.
- **배포된 사이트에서 다시 잰다.** 로컬에서 맞는 것으로 끝내지 않는다 — 프로덕션 번들은
  minify·청킹이 다르다. 직전 확인(P29)에서 배포본이 반팔 18/18 · 긴팔 17/17 채널을
  **정착 해시까지 비트 동일**하게 재현했다.

### 캠페인 로그 — `.obsidian-log/`

드레이프 물리를 고치려던 **108회차의 실험 기록**이 Obsidian 볼트로 남아 있다.
회차마다 표적 · 사전 등록한 판정 갈래 · 실측 · 판정 · 등재가 있고, 반복해서
밟은 함정 33가지가 별도 노트로 정리돼 있다(계기가 이름과 다른 것을 재고 있었다,
...
| **커프 밴드 — 기본 off** | 위상 리팩터는 끝났고(P18) 밴드는 **선다** — 제도·조립·배치가 서고 반팔 2cm·5cm는 착장이 DONE에 도달한다. 그런데 **긴팔은 여전히 착장에 실패**하고(S1 정체), 반팔 밴드 on도 **실행마다 정착이 갈린다**(메시는 같다 · 정착만 다르다). 원인 미규명이라 밴드 값을 기준선으로 등재하지 않았고 **기본값 0**으로 둔다. 코드는 남긴다 |
| **관통 1~2%** | 정착 후 옷 정점의 1~2%가 몸 안쪽으로 판정된다(반팔 51/5204 · 긴팔 107/6294). **자리는 특정됐다** — 87~93%가 소매이고 소매 안에서 **암홀 대역과 소맷부리 대역 두 곳뿐**이다(중간 구간은 정확히 0). **원인은 미규명이고 처방은 없다** |
| **`round()` 경계가 증폭기다** | 소매 표본 수를 `round(total)+1`로 정하는 자리가 있어, 입력이 ULP 단위로만 흔들려도 **정점 수가 ±2** 바뀌고 삼각화·엣지·배치·정착이 전부 따라 갈린다. 기본 티셔츠는 그 경계에서 멀어 재현되지만 **구조적 안전은 아니다**(P21·P22) |
| **세션 누적 — 미재측정** | 같은 로드 안에서 실행을 거듭하면 몸이 조금씩 달라지는 현상이 있었다(자기교차 2143→2136). P27 이후로는 완화됐을 것으로 보이지만 **다시 재지 않았다** — 「완화됐다」가 아니라 **미측정**이 현재 상태다 |
| **백그라운드 탭에서는 시작할 수 없다** | 착장 루프 자체는 워커의 동기 루프라 탭 가시성과 무관하다. 그런데 **시작 조건**은 마네킹 포즈가 멎었는지를 보고, 그 포즈 루프는 메인 스레드 rAF다 — 백그라운드 탭은 rAF가 멎어 프레임이 0이고 「착장하기」가 안 켜진다. 탭을 전경으로 되돌리면 정상 진행되므로 **갇히지는 않는다**. 5초를 넘기면 화면이 **사유를 말한다**(P30) — 그 전에는 「불러오는 중」에 머문 채 침묵했다. **준비 안 된 채로 굽지는 않는다**: 그렇게 구우면 결정성이 되살아난다 |
| **조립 층 잔여 고정 1곳** | 패널 배열을 전역 버퍼로 합치는 층에 티셔츠 전제가 남아 있다 — `necklineRing`이 **패널 0·1에 고정**이다. 같은 층의 다른 고정(2D 좌표·삼각형·UV·엣지의 `p < 4`, 자기충돌 문턱, 삼각화 품질 배열)은 P20·P21이 걷어냈다 |
...
| **몸 좌표 민감도** | 몸 정점이 0.14mm만 달라도 자기교차가 13% 움직인다 — **민감도 자체는 그대로**다. 다만 P27 이후 같은 조건이면 몸 스냅샷이 비트 동일하게 재현되므로 실사용에서 드러나지 않는다. 기준선 B-3의 재현 조건은 그만큼 넓어졌다(새 로드 기본값 **+ 슬라이더를 움직였다 되돌린 뒤에도**) |
| **옷 타입 1종** | 티셔츠(반팔·긴팔)뿐이다. 셔츠·바지는 **위상**이 달라(닫힌 목선 · 어깨 매달림 · 몸판2/소매2 전제) 큰 작업이다 — `docs/product-axis/P7-scope.md`. 선행 리팩터(소비 층 일반화)는 P18·P19가 끝냈고 **남은 티셔츠 전제는 생산 층 9곳**이다(P19 §6에 전량) |
...
핏 맵·렌더 수리(P17) · 위상 일반화(P18) · 커프 밴드(P19) · 밴드 갭 진단(P20) ·
조립 층과 비결정성(P21) · 비트 해시(P22) · 굽기 시점(P23) · 조용한 실패 점검(P23b) ·
A포즈 고정(P24) · `bodyReady` 수리(P25) · 스케일 대입(P26) · 절대 회전(P27) ·
캡처 보강(P27b·P27c) · 기준선 재수립(P28) · 매듭 갱신(P29).

**전제가 뒤집힌 판이 넷 있다**(그것도 결과다): P10은 「관통 2배는 전완 콜라이더 탓」을
반증했고, P14는 「눌림과 관통은 다른 채널」을 반증했으며(완전히 같은 집합이었다),
P15는 「옷 어깨너비가 미배선」이라는 전제 자체를 반증했고(이미 배선돼 있었다),
P17은 「핏 맵 토글이 다른 렌더 경로로 보낸다」를 반증했다(경로 분기가 아예 없었다).

**사전 등록한 갈래 어디에도 안 걸린 판이 둘 있다**: P20과 P21이다. 둘 다 「판정 불가」로
적고 멈췄다 — 갈래를 사후에 늘려 맞추지 않는다. P20의 원인은 갈래가 상정한
「제도→배치→시접」 **사이의 조립 층**이었고, P21은 A를 가를 정밀도가 계기에 없었다.
...
### 반복해서 밟은 함정 — 아래는 15건 발췌

볼트 정본은 **33건**이다(`.obsidian-log/함정/`). 아래 15건은 v1 캠페인에서 나온 것들로,
측정 쪽 9건 · 제어 쪽 1건 · 측정 대상 쪽 1건 · 시간 쪽 1건 · 기록 쪽 1건 · 방법 쪽 1건이다.
전부 실제로 당한 것이고, 각각 최소 한 번은 "지표는 통과인데 화면은 실패"로 이어졌다.
