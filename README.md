...
측정 쪽 6건, 제어 쪽 1건, 측정 대상 쪽 1건, 시간 쪽 1건. 전부 실제로 당한 것이고, 각각 최소 한 번은
...
- **소매캡 틈**: 어깨 큰 자세에서 소매캡과 몸판 사이가 벌어져 맨살이
  비친다(캡처의 겨드랑이 부근). 원인은 소매산(sleeve cap) volume 부재 —
  M1 백로그.
- **뒤 목선 국소 구김**: 등 상단 중앙에 좁은 구김/노출이 남아 있다.
  넥밴드 작업 전후로 불변(렌더 전용이므로 당연) — 물리 층 백로그.
- **구 플랩 열**: 몸통 범위 밖 열이 렌더에서는 제외되지만 물리로는 계속
  돈다 — 성능·측정 노이즈 요인.
