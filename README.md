...
![핏 리포트](docs/captures/P9-매듭/배포-착장완주.jpg)
...
| 몸 실측 | `src/lib/bodyMeasure.ts` | 1cm 슬라이스로 가슴·허리·목밑·어깨 통과 둘레와 어깨 능선을 잰다 |
...
| **긴팔 마감** | 긴팔은 동작하나 **커프가 없고**(소맷부리가 열린 원통) **전완 콜라이더가 없다**(팔 캡슐 2개가 같은 직선축이라 팔꿈치를 안 따른다 — 관통 49→103) |
| **소매 핏 측정 불가** | 핏 리포트가 소매를 못 잰다. `signedClearance`가 보는 몸통 메시에 팔이 없다 |
| **옷 어깨너비 미배선** | 「어깨너비(옷)」 슬라이더가 제도에 도달하지 않는다. 어깨 핀이 마네킹 포즈에서 나오고 그 값이 슬라이더 기본값과 달라서, 배선하면 기본값에서 기준선이 깨진다 |
