...
function runFixture(path: string): void {
  ...
  console.log(`  columnRipple(열 방향 공간 4차차분 · 최종프레임 1장 · 정규화 없음): max ${ripple.columnRippleMaxMm}mm @ ${JSON.stringify(ripple.columnRippleMaxAt)} / mean ${ripple.columnRippleMeanMm}mm / 부호반전 ${ripple.signFlipRatio}`);
  // COLRIPPLE_DUMP=1 — columnRipple 지표의 분해(계기 검증 + 공간 국소화). 물리 무변경,
  // 최종 프레임 위치만 읽는다. columnRipple은 **시간 차분이 아니라 열 방향 공간
  // 4차 차분**이므로 이 덤프는 "어느 정점이 그 합을 만드는가"만 답한다.
  if (process.env.COLRIPPLE_DUMP === "1") {
    const per = columnRipplePerVertex(sim, [PANEL_FRONT, PANEL_BACK], 1, armholeStartRow, xMin, xMax);
    const mean = per.reduce((a, b) => a + b.mm, 0) / (per.length || 1);
    // 계기 동일성 — 이 덤프의 평균이 게이트 지표와 같은 수인지(0.01mm 이내).
    const drift = Math.abs(mean - ripple.columnRippleMeanMm);
    console.log(`  [CRDUMP] 창: 패널 앞/뒤 · row1..${armholeStartRow} · 열 ${xMin + 2}..${xMax - 2} / 표본 ${per.length} · 평균 ${mean.toFixed(2)}mm (게이트값과 차 ${drift.toFixed(3)}mm ${drift <= 0.01 ? "OK" : "불일치!"})`);
    ...
    console.log(`  [CRDUMP] 분포(mm): min ${q(0)} p25 ${q(0.25)} med ${q(0.5)} p75 ${q(0.75)} p90 ${q(0.9)} p95 ${q(0.95)} max ${q(1)}`);
    // 상위 5% 집합 J — 합 기여도와 위치.
    ...
    console.log(`  [CRDUMP] J(상위5%, >=${cut.toFixed(2)}mm): n=${J.length} / 총합기여 ${share.toFixed(1)}%`);
    console.log(`  [CRDUMP] J 목록: ${J.sort((a, b) => b.mm - a.mm).map((v) => `${v.panel === PANEL_FRONT ? "F" : "B"}${v.x},${v.y}=${v.mm.toFixed(1)}`).join(" ")}`);
    // 행·열·패널 집계.
    ...
    console.log(`  [CRDUMP] 패널별 평균: ${agg((v) => (v.panel === PANEL_FRONT ? "앞" : "뒤"))}`);
    console.log(`  [CRDUMP] 행별 평균:   ${agg((v) => `row${v.y}`)}`);
    console.log(`  [CRDUMP] 열별 평균:   ${agg((v) => `x${v.x}`)}`);
    ...
    console.log(`  [CRDUMP] 창 평균 열간격 ${((sp / spN) * 1000).toFixed(2)}mm / 순수교대 환산진폭 = 평균columnRipple/16 = ${(mean / 16).toFixed(3)}mm`);
  }
