/* v5-19 §1-① — **S3-only(S4 없음) f0 사실**(측정만 · `src/` 거동 0줄 · 물리 0프레임 · 판정 0).
 *
...
const out = {
  what: 'v5-19 §1-① S3-only(S4 없음) f0 사실 — 교차·관통·간격·봉제쌍·링(측정만 · src 거동 0줄 · 판정 0)',
  /** ★ v5-18 — **진입 env 를 산출에 «적는다»**(CC 귀책 C3 재발 방지 · v5-7b §0-5ㄴ 확정분인지 눈으로 본다). */
  _args: { ASM2FIX: FIX ?? null, SPEC: SPEC ?? null, CELL, D_MM: D * 1000, 계기: 'v5NoS4.ts',
  ...
  '★ ① 관통': a2?.['관통'] ?? null,
  '① 목선 링/rest': a2?.['목선 링'] ?? null,
  '① 소매 관련 교차': rows.filter((r) => r.panPair.includes('sleeve')).length,
  '① 몸판끼리 교차': rows.filter((r) => !r.panPair.includes('sleeve')).length,
  FIX: a2?.FIX ?? null,
};
writeFileSync(`gpu/oracle/export/v5-19-nos4-${FIX ?? 'DEF'}-${SPEC ?? CELL}.json`, JSON.stringify(out, null, 1));
