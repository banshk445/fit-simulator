...
## §5 후속 정정 — P10 실측 (원문 무삭제 · 2026-08-10)

**§2-1 「미흡한 점 2가지」에 대한 정정 3건.** 정본은 `docs/product-axis/P10-긴팔마감.md`.

1. **「제도에도 커프 세그먼트가 없다」 — 거짓.** `patternDraft.ts:395`에 이름이 그대로
   `"cuff"`인 세그먼트가 **이미 있다**(소매 패널 닫힌 루프의 밑변). 없는 것은 세그먼트가
   아니라 **그 세그먼트의 시접 짝**이고, `SeamSpec`은 세그먼트 «쌍»이라 `kind` 한 줄
   추가로는 아무 일도 일어나지 않는다 — **커프 밴드 패널이 새로 필요하다**.
2. **「관통 49→103의 자리가 전완 콜라이더로 보인다」 — 반례.** P10 §1이 캡슐을 팔꿈치에서
   꺾어 전완을 따르게 했는데 **관통은 103 → 105**다(줄지 않았다). 다만 자기교차는
   2159 → 1872(−13.3%)로 개선됐고 화면에서 소매가 전완을 따라간다.
3. **커프의 크기 산정 「반나절~P4급」 — 과소.** 커프 둘레·밴드 높이를 도출할 몸 실측이
   없다(`BodyMeasure`에 **손목 둘레가 없고** `dims.armGirthM`은 하드코딩 0). 새 상수 없이는
   집행 불가라 **손목 둘레 계기가 선행**해야 한다.
